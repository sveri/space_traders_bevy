...
pub(crate) fn handle_waypoint_selected_event(
    mut event: EventReader<WaypointSelected>,
    waypoint_query: Query<(Entity, &Waypoint)>,
    ...
    for select_event in event.iter() {
        if let Ok((_entity, waypoint)) = waypoint_query.get(select_event.0) {
            select_waypoint_text.single_mut().sections[0].value = format!("Selected Waypoint: {}, Traits: {}", waypoint.symbol, waypoint.get_traits());
        }
    }
}
